...
function share_init()
{
    //alert(browserInfo.versions.mobile);
    //alert(browserInfo.versions.canShare);
    if (true == browserInfo.versions.mobile && browserInfo.versions.canShare)
    {
        $('.nativeShareWrapper').show();
    }
    else
    {
        $('.nativeShareWrapper').hide();
    }
}
...
function getLbsLocation(action) {
    ...
    if (navigator.geolocation) {
        ...
        var url;
        url = "http://api.map.baidu.com/getscript?v=2.0&ak={0}&services=&t=20161130153924";

        url = "https://api.map.baidu.com/getscript?v=2.0&ak={0}&s=1&services=&t=20161130153924";

        var defaultAk = "XreLZP0GQOx8BZV7ZFEX9BPlh4dEWZvV";
        //alert(baiduMapAk);
        var ak = baiduMapAk;
        if ("" == baiduMapAk) {
            ak = defaultAk;
...
function onLbsPrepared() {
    ...
    geolocation.getCurrentPosition(function (r) {
        if (this.getStatus() == BMAP_STATUS_SUCCESS) {
            ...
            if ("seller" == lbsAction) {
                //alert('seller');
                jumpNearbySellerList(r);
            }
            else if ("order" == lbsAction) {
                jumpNearbyOrderList(r);
            }
...
function setSelectedRadioButtonListIndex0(id, index) {
    var children;
    var string = '#' + id + ' input';
    children = $(string);
    for (var i = 0; i < children.length; i++) {
...
//radiobuttonlist
function decorateRadioButtonList() {
    var id;
    var arr = $('.radiolist');
...
{
    //$("#spanDebugInfo").text(s);
}

function gotoTop(min_height) {
    //预定义返回顶部的html代码，它的css样式默认为不显示
    var gotoTop_html = '<div id="gotoTop" class="iconbg icon_top"><em></em></div>';
...
function showShare() {
    ...
    var shareJsCommonDiscuz = "/scripts/common_discuz.js?v=2017050601";

    var shareJs = "/scripts/m/nativeShare.js?v=2017050612";
    var shareCss = "/css/m/nativeShare.css?v=2017050602";
    
    head.load(shareJsCommonDiscuz,shareJs, shareCss);
    head.ready(function () {
        _showShare();
    });
...
function _showShare() {
    ...
    var url = "/window/share";
    url = url + "?callbackfun=shortcutCallbackfun&random=" + Math.random();
    v = { 'maxh': 600, 'pos': '00', 'cache': 0, 'duration': 2, 'timeout': 2000000 };
    showWindow("shortcut", url, 'get', -1, v);
...
function setSingleCheckBoxState(wapperId, checked) {
    var children;
    var string = '#' + wapperId + ' input';
    children = $(string);

    $(children[0])[0].checked = checked;
    changeCheckBox($('#' + wapperId)[0], 0);
}
...
function getSelectedDecoroateCheckBoxListText(ctrl) {
    var ret = "";
    var id = ctrl.attr("id");
    var string = '#' + id + ' input';
    children = $(string);
...
function search() {
    ...
    if ("" != mostPossibleLink) {
        ...
        if (userNameInputTip == keyword || "" == keyword) {
            ...
            url = "/sellerlist/keyword" + keyword + "";
        }
...
function keywordComplete(searchField) {
    var inputValue = '';
    $("#TextBoxKeyword").autocomplete(null, {
        ...
        max: 20,
        width: 280,
        extraParams: { searchField: searchField, para2: 'test' },
        url: '/http/search',
        urlparaname: 'keyword',
        scrollHeight: 280,
...
function showLiveChatBar(show)
{
    var liveChatBar = $("#liveChatBar");
    if (liveChatBar.length > 0)
    {
        liveChatBar.addClass("hide");
    }
}

function showLiveChatBarbody(show)
{
...
function showPromptWithStyle(ctrlid, evt, msg, timeout, v) {
    var typeClass = "";
    var divStyle = "";
    var defaultType = 1;//warn
    var type;
    if (isUndefined(v)) {
        type = defaultType;
    }
    else {
        type = isUndefined(v['type']) ? defaultType : v['type'];
    }

    //alert(type);
    switch (type) {
        case 1://warn
            typeClass = 'icon_warning';
            break;
        case 2://success
            typeClass = 'icon_success';
            break;
        case 3://error
            typeClass = 'icon_error';
            break;
        case 4://prompt
            typeClass = 'icon_prompt';
            break;
    }

    divStyle = '<div class="iconbg' + ' ' + typeClass + '"></div>';
...
function makeLinkResponsive() {
    if (false == browserInfo.versions.mobile) {
        var items = $(".responsive a");
        for (var i = 0; i < items.length; i++) {
            var target = $(items[i]).attr("target");
            var href = $(items[i]).attr("href");
            if ("undefined" == typeof (href) || 0 == href.indexOf("javascript:")) //no href
            {
                continue;
            }
            else {
